...
def prepare_for_editor(df: pd.DataFrame) -> pd.DataFrame:
    """给 data_editor 使用：强制列类型稳定，避免 schema 检查报错。"""
    ...
    # 类型规范化（避免奇怪值）
    x["类型"] = x["类型"].apply(lambda v: v if v in ["收入", "支出"] else v.strip())
...
with st.sidebar.form("record_form", clear_on_submit=True):
    ...
    if submit:
        try:
            ...
            st.session_state.records = prepare_for_editor(st.session_state.records)
            persist_all()
...
# =========================
st.title("💰 我的财务一体化看板")
df0 = enrich_records(prepare_for_editor(st.session_state.records))
...
with tab1:
    ...
    if full.empty:
        ...
        st.caption(f"当前显示：{len(view)} 条（0 条时不会渲染编辑器，避免报错）")

        # ✅ 关键修复：0 条就不要 data_editor
        if view.empty:
            st.info("当前筛选条件下没有记录。请清空筛选或换关键词。")
        else:
            edited = st.data_editor(
                ...
                column_config={
                    ...
                    # 账本/类别/项目用 TextColumn 最稳（不会因为选项不包含导致类型检查失败）
            ...
            with colC:
                ...
                st.caption("自动保存位置：data/records.csv（改代码/重启后自动加载）")


# -------------------------
# Tab2 (import + stats + budget)
# -------------------------
with tab2:
        ...
        if up is not None:
            try:
                ...
                if st.button("✅ 解析并导入", key="import_file_btn"):
                    ...
                    tmp["类别"] = df_in.loc[tmp.index, col_cat].astype(str).fillna("其他") if col_cat != "<无>" else "其他"
                    tmp["账本"] = df_in.loc[tmp.index, col_book].astype(str).fillna("生活主账") if col_book != "<无>" else "生活主账"
                    tmp["项目"] = df_in.loc[tmp.index, col_item].astype(str).fillna("") if col_item != "<无>" else ""
    ...
    with imp_tab2:
        ...
        if st.button("✅ 解析文本并导入", key="import_memo_btn"):
            ...
            if df_m.empty:
                st.warning("没有解析出有效记录：请确保每行至少包含日期 + 金额。")
            else:
                start = next_id()
                df_m.insert(0, "ID", range(start, start + len(df_m)))
                df_m = prepare_for_editor(df_m.assign(ID=df_m["ID"])[RECORD_COLS])

                st.session_state.records = prepare_for_editor(pd.concat([prepare_for_editor(st.session_state.records), df_m], ignore_index=True))
                persist_all()
